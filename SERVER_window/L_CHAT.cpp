...
#pragma region 통신
		...
		try {
			...
			} while (recvMsgSize > sizeof(int)&& user->getRunner());
...
#pragma endregion
	...
	while (RUNNER) {
		...
		for (i = 0; i < MAXIMUM_USER; i++) {
			...
			if (isSAVE[i] == SAVED) {																// 수신되는 이미지가 존재할 경우
...
#pragma region 기본 출력문 & 변수 선언 및 초기화
	...
	cout << "[EVENT] 슬픔 : " << EventOption[4] << endl;
	cout << "[EVENT] 놀람 : " << EventOption[5] << endl;
	cout << "[EVENT] 자연 : " << EventOption[6] << endl;

	thread t_ai = thread(OJJJ_Thread_AI, PORT);

	cmd_ctr.Setting_User(PORT);
...
#pragma region 사용자 정상 요청 부분
						...
						if (Socket_Buffer[0] == '$') {													// 이미지 수신 요청 
							...
							*cmd_ctr.Get_user_id(read.fd_array[i])->get_thread(target_id)
							...
							sprintf(msg, "!,%d\n", read.fd_array[i]);
							for (int j = 1; j < read.fd_count; j++) {
								cout << "send to -> " << read.fd_array[j];
								send(read.fd_array[j], msg, strlen(msg), 0);
							}
