...
# - 0818
 ...
 -- 감정인식 결과에 따라서 적절한 Event만 추가하면 됨
#
# - 0810
 - 서버 및 클라이언트의 발견될 수 있느 모든 예외처리 잡음
 
 -- UI 프로세스 상태에 따른 적절한 이벤트? 애니메이션이 발생하도록 수정 해야함
 -- 클라이언트 이름 설정하는 부분은 설정하지 않음
#
# - 0804
 - 서버, 클라이언트 자잘한 예외처리 작업중
 [클라이언트가 대화방에서 나갔다가 다시 들어왔을 때 예외처리 -> 예외발생한 이유는 모름. 그냥 스레드 아이디를 찍으니 에러가 안남]
 [클라이언트가 모두 접속을 종료할때 여러 스레드들이 정리되도록 서버에서 작업중]
 -- 클라이언트의 이미지를 서버 메모리에 저장하는 부분에서 에러가 발생함. 서버가 종료하지 않는경우는 에러가 나지 않음.
 -- 스레드 정리부터 할 것. 다중 프로세스간에 데이터 전송법을 공부해서 대기방에 접속하는 클라이언트 숫자 체크, 대기방이 삭제되는지 파악해보아야함.
#
# - 0710
 - 통신안정화 > 성공한듯 보임
  [UDP -> TCP로 변경]
  [소켓에 데이터가 쌓이는것도 문제로 보임 -> 수신은 딜레이 없이, 송신은 딜레이 있이 만듦]
  [이미지 변환에서 데이터가 모자르거나 많아도 에러가 발생하는 것 같아서 정확한 패킷이 송신되는거 아니면 변환자체를 안하도록 막음]
 - 여기까지는 문제 없어 보임

 -- 코드 정리 및 다른 기능 추가 보완 시작해야함
#
# - 0627
 - 인공지능과의 연결 확인 > 성공
